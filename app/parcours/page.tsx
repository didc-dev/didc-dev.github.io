...
export default function ParcoursPage() {
  return <>
    ...
    <section className="section section-shell"><div className="timeline">{experiences.map((item, index) => <article key={`${item.period}-${item.company}`}><div className="timeline-marker" aria-hidden="true" /><div className="timeline-date">{item.period}</div><details className="timeline-content" open={index < 2}><summary><ResponsiveImage imageKey={timelineImages[index]} className="timeline-image" sizes="(max-width: 680px) 100vw, 720px" /><span>{item.domain}</span><h2>{item.role}</h2><p>{item.company}</p></summary><p>{timelineNotes[index]}</p></details></article>)}</div></section>
